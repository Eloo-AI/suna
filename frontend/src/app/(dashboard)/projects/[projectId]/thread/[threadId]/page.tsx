...
export default function ThreadPage({
  ...
  useEffect(() => {
    if (projectName) {
              document.title = `${projectName} | Eloo AI`;
      ...
      if (metaDescription) {
        metaDescription.setAttribute(
          'content',
          `${projectName} - Interactive agent conversation powered by Eloo AI`,
        );
      }

      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | Eloo AI`);
      }
  ...
  useEffect(() => {
    if (initialLoadCompleted && subscriptionData) {
      const hasSeenUpgradeDialog = localStorage.getItem('eloo_ai_upgrade_dialog_displayed');
  ...
  const handleDismissUpgradeDialog = () => {
    setShowUpgradeDialog(false);
              localStorage.setItem('eloo_ai_upgrade_dialog_displayed', 'true');
  };
        ...
        <div
          ...
          <div className={cn(
            ...
            <ChatInput
              ...
              placeholder={`Ask ${agent ? agent.name : 'Eloo AI'} anything...`}
              loading={isSending}
